...
public class NetworkAggregator extends BaseHardwareDocAggregator {

    public NetworkAggregator(InventoryReport inventoryReport) {
        super(inventoryReport);
    }

    @Override
    public InventoryReport.Column column() {
        return InventoryReport.Column.NETWORK;
    }

    @Override
    public void handleSheetDoc(Document doc) {
        BaseDocAggregator.Value value = BaseDocAggregator.Value.of(doc.get(Context.Field.VALUE), doc.get(Context.Field.STOPPED));
